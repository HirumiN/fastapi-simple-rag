...
def retrieve_similar_rags(db: Session, query_vector: List[float], top_k: int) -> List[models.RAGSEmbedding]:
    """Performs similarity search in the database using pgvector."""
    # Use the native l2_distance operator from pgvector.sqlalchemy
    result = db.query(models.RAGSEmbedding).order_by(
        models.RAGSEmbedding.embedding.l2_distance(query_vector)
    ).limit(top_k).all()
    
    return result

def augment_prompt(question: str, context_docs: List[models.RAGSEmbedding]) -> str:
